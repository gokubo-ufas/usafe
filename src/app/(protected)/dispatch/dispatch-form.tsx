...
export function DispatchForm({ employeeCount }: { employeeCount: number }) {
  ...
  async function fetchEmployees() {
    setLoadingEmp(true)
    setEmpMessage('社員情報を取得中…')
    try {
      const res = await fetch('/api/employees/active')
      if (!res.ok) throw new Error()
      const data = await res.json() as Employee[]
      setEmployees(data)
      setEmpMessage(`${data.length}名の在籍社員情報を取得しました`)
    } catch {
      setEmpMessage('取得に失敗しました。再度お試しください。')
    } finally {
      setLoadingEmp(false)
    }
  }
    ...
    <>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2 bg-gray-50 rounded-xl px-4 py-2.5 border border-gray-100">
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">発報対象</span>
              <span className="text-sm font-bold text-gray-800 tabular-nums">{employeeCount}名</span>
              <span className="text-xs text-gray-400">（在籍中）</span>
            </div>
            <button
              type="button"
              onClick={fetchEmployees}
              disabled={loadingEmp}
              className="text-xs font-medium text-emerald-700 hover:text-emerald-800 disabled:opacity-40 transition-colors shrink-0"
            >
              {employees ? '再取得' : '社員一覧を確認'}
            </button>
          </div>

          {empMessage && (
            <p className={`text-xs px-1 ${loadingEmp ? 'text-gray-400' : employees ? 'text-emerald-600' : 'text-red-500'}`}>
              {empMessage}
            </p>
          )}

          {employees && (
            <div className="bg-gray-50 rounded-xl border border-gray-100 divide-y divide-gray-100 max-h-[36rem] overflow-y-auto">
              {employees.map((emp) => (
                <div key={emp.employee_number} className="grid grid-cols-3 px-4 py-2 text-xs">
                  <span className="text-gray-400 truncate">{emp.department ?? '—'}</span>
                  <span className="text-gray-500 truncate tabular-nums">{emp.employee_number}</span>
